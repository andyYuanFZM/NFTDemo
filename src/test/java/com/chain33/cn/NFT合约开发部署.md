...
2. 关于链上的燃料费：
&nbsp; 2.1 BTY平行链场景：  发往BTY平行链上的每一笔交易都需要扣除燃料费，燃料费计算和扣除方式见BTYNFT目录下的readme文档。    
&nbsp; 2.2 YCC平行链场景：  发往YCC平行链上的每一笔交易都需要扣除燃料费，燃料费计算和扣除方式见YCCNFT目录下的readme文档。    
&nbsp; 2.3 solo场景：这种场景用于开发和测试，默认配置是关闭手续费功能，交易上链不用考虑手续费的问题。  
&nbsp; 2.4 联盟链的场景： 联盟链在实际生产部署时，默认是关闭手续费的功能的，所以交易上链不用考虑手续费的问题。 
